...
let activeSectionKey: string | null = null; // 跟踪当前打开的 section

interface SectionContext {
	isOpen: boolean;
	toggle: () => void;
	collapseOthers: (selfKey: string) => void;
	collapsible: boolean;
}
...
export function Section({
	...
	// 如果有 sectionKey 且 activeSectionKey 不是当前 section，则默认关闭
	const initialOpen = sectionKey 
		? (cached !== undefined ? cached : (activeSectionKey === null || activeSectionKey === sectionKey))
		: defaultOpen;
	...
	const toggle = () => {
		...
		setIsOpen(next);
		if (sectionKey) {
			sectionExpandedCache.set(sectionKey, next);
			if (next) {
				// 打开时，关闭其他所有 section
				activeSectionKey = sectionKey;
				sectionExpandedCache.forEach((_, key) => {
					if (key !== sectionKey) {
						sectionExpandedCache.set(key, false);
					}
				});
			}
		}
	...
	const collapseOthers = (selfKey: string) => {
		activeSectionKey = selfKey;
		sectionExpandedCache.forEach((_, key) => {
			if (key !== selfKey) {
				sectionExpandedCache.set(key, false);
			}
	...
	return (
		<SectionCtx.Provider value={{ isOpen, toggle, collapsible, collapseOthers }}>
			<div
				className={cn(
					"flex flex-col",
